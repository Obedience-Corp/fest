...
func TestTemplateResolver_Cache(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(templateDir, "CACHED.md"), []byte("# Cached"), 0644)
...
func TestTemplateResolver_HierarchyPrecedence(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(globalDir, "TEST.md"), []byte("builtin"), 0644)

	// Festival
	festivalTemplateDir := filepath.Join(festivalPath, ".festival", "templates", "gates")
	_ = os.MkdirAll(festivalTemplateDir, 0755)
	_ = os.WriteFile(filepath.Join(festivalTemplateDir, "TEST.md"), []byte("festival"), 0644)

	// Phase
	phaseTemplateDir := filepath.Join(phasePath, ".fest.templates")
	_ = os.MkdirAll(phaseTemplateDir, 0755)
	_ = os.WriteFile(filepath.Join(phaseTemplateDir, "TEST.md"), []byte("phase"), 0644)

	// Sequence
	seqTemplateDir := filepath.Join(sequencePath, ".fest.templates")
	_ = os.MkdirAll(seqTemplateDir, 0755)
	_ = os.WriteFile(filepath.Join(seqTemplateDir, "TEST.md"), []byte("sequence"), 0644)
...
func TestTemplateResolver_ResolveForFestival_GatesPrefix(t *testing.T) {
	...
	_ = os.WriteFile(gatesTemplatePath, []byte("# My Gate"), 0644)
...
func TestTemplateResolver_ResolveForPhase_GatesPrefix(t *testing.T) {
	...
	_ = os.WriteFile(gatesTemplatePath, []byte("# Phase Gate"), 0644)
